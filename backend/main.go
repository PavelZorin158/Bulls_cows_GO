...
func createTableDB() {
	...
	db, err := sql.Open("sqlite3", "../dir_db/cow.db")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
...
func new_game(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || n <= 0 || n > ConfigGame.MaxN {
...
func newConfig() {
	...
	err = ioutil.WriteFile("../dir_db/config.json", data, 0600)
	if err != nil {
		panic(err)
	}
}
...
func loadConfig() {
	data, err := ioutil.ReadFile("../dir_db/config.json")
	if err != nil {
		if err.Error() == "open ../dir_db/config.json: The system cannot find the file specified." {
...
func main() {
	...
	fmt.Println("Сервер запущен на 5001 порту")
